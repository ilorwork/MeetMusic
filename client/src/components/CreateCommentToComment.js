...
const CreateCommentToComment = ({ comment, getCommentsOfComment, commentsToCommentCount,
    setCommentsToCommentCount, paddingXForCommentToComment }) => {
    const [contentOfCommentToComment, setContentOfCommentToComment] = useState("");
    ...
    const handleCreatingCommentToComment = async () => {
        ...
        try {
            const res = await axios.post("http://localhost:7000/comments-to-comments/", newCommentToComment,
                {
    ...
    return (
        <FormControl sx={{ pl: paddingXForCommentToComment }}>
            <Textarea
                ...
                onKeyDown={(e) => { if (e.key === "Enter") handleCreatingCommentToComment() }}
                endDecorator={
                    <Box
                        sx={{
                            display: 'flex',
                    ...
                    >
                        <Button sx={{ background: "rgb(38, 165, 165)", color: "white", p: 1, fontSize: 10 }} variant={"contained"}
                            onClick={handleCreatingCommentToComment}>Send</Button>
                    </Box>
                }
                sx={{
                    minWidth: 200
