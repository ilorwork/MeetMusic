...
const addLike = async (req, res) => {
    try {
        ...
        return res.status(201).json({ newLike: newLike, postLikesAmount: likedPost.likesCount });
    } catch (e) {
...
const removeLike = async (req, res) => {
    try {
        const creator = await UserModel.findOne({ email: req.user.email });
        req.body.creator = creator._id;
        const removedLike = await LikeModel.deleteOne({ creator: req.body.creator, postId: req.body.postId });
        const unLikedPost = await PostModel.findOne({ _id: req.body.postId });
        unLikedPost.likesCount -= 1;
        await unLikedPost.save();
        return res.status(200).json({ removedLike: removedLike, postLikesAmount: unLikedPost.likesCount });
    } catch (e) {
...
const isUserLikeThePost = async (req, res) => {
    try {
        const creator = await UserModel.findOne({ email: req.user.email });
        req.body.creator = creator._id;
        const isUserLikeThePost = await LikeModel.findOne({ creator: req.body.creator, postId: req.body.postId });
