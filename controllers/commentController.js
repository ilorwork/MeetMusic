...
const createComment = async (req, res) => {
    try {
        ...
        return res.status(201).json({ newComment: newComment, commentsCount: postCommented.commentsCount });
    } catch (e) {
...
const deleteComment = async (req, res) => {
    try {
        const commentToDelete = await CommentModel.findOne({ _id: req.body._id }).populate(
            "creator"
        );
        ...
        postCommented.commentsCount--;
        await postCommented.save();
        return res.status(200).json({ deletedComment: deletedComment, commentsCount: postCommented.commentsCount });
    } catch (e) {
